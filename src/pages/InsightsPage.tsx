...
import { RefreshCw, FileDown, MessageSquare, Brain, Inbox, Upload, Send, Bot, User, Loader2 } from "lucide-react";
...
const InsightsPage = () => {
  ...
  const buildDataContext = async (): Promise<string> => {
    ...
    if (salesRes.data?.length) {
      ...
      parts.push(`SELL-OUT SUMMARY (latest ${salesRes.data.length} rows): Total Revenue: $${totalRev.toLocaleString()}, Total Units: ${totalUnits.toLocaleString()}, Retailers: ${retailers.join(", ") || "N/A"}, Brands: ${brands.join(", ") || "N/A"}`);
    }
    ...
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={fetchReport}><RefreshCw className="h-3.5 w-3.5 mr-1.5" />Regenerate</Button>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground mb-3">Insights will appear here after your data has been processed by the 5-agent pipeline.</p>
